...
import { DatabaseHandler } from './dist/DatabaseHandler.js';
...
async function runTests() {
  ...
  try {
    ...
    console.log(`   - Total Tasks: ${snapshotData.feature?.totalTasks || taskIds.length}`);
    console.log(`   - Progress: ${snapshotData.feature?.progress || 'N/A'}`);
    console.log(`   - Blockages: ${snapshotData.blockages?.length || 0}`);
    console.log(`   - Compressed size: ${JSON.stringify(snapshotData).length} bytes\n`);
    ...
    const optimalOrder = planData.optimalOrder || [];
    const parallelizable = planData.parallelizable || {};
    const criticalPath = planData.criticalPath || [];
    const warnings = planData.warnings || [];
    console.log(`   - Optimal Order: ${optimalOrder.join(' → ') || 'N/A'}`);
    console.log(`   - Parallelizable Phases: ${Object.keys(parallelizable).length}`);
    console.log(`   - Critical Path: ${criticalPath.join(' → ') || 'N/A'}`);
    ...
    for (const taskId of taskIds) {
      for (const role of reviewRoles) {
        ...
        const validation = await manager.validateReviewCompleteness({ repoName: REPO_NAME, featureSlug: FEATURE_SLUG, taskId, stakeholder: role });
        ...
        const reviewResult = await manager.addReview({
          repoName: REPO_NAME,
          featureSlug: FEATURE_SLUG,
          taskId,
          stakeholder: role,
          decision: 'approve',
          notes: `Thorough review completed by ${role}`,
          additionalFields
        });
    ...
    const checkpoint1 = await manager.saveWorkflowCheckpoint({
      repoName: REPO_NAME,
      featureSlug: FEATURE_SLUG,
      description: 'All tasks refined and approved - ready for development'
    });
    ...
    const devBatch = await manager.batchTransitionTasks({
      ...
      toStatus: 'InProgress',
      actor: 'developer',
      metadata: { developerNotes: 'Implemented all features', filesChanged: ['src/main.ts', 'src/api.ts'] }
    });
    ...
    const checkpoint2 = await manager.saveWorkflowCheckpoint({
      repoName: REPO_NAME,
      featureSlug: FEATURE_SLUG,
      description: 'Developer implementation complete - all tasks in InProgress'
    });
    ...
    const reviewBatch = await manager.batchTransitionTasks({
      ...
      toStatus: 'InReview',
      actor: 'codeReviewer',
      metadata: { codeReviewerNotes: 'Code quality verified' }
    });
    ...
    const finalizeBatch = await manager.batchTransitionTasks({
      ...
      toStatus: 'Done',
      actor: 'qa',
      metadata: { qaNotes: 'All tests passed successfully' }
    });
    ...
    const checkpoint3 = await manager.saveWorkflowCheckpoint({
      repoName: REPO_NAME,
      featureSlug: FEATURE_SLUG,
      description: 'All tasks completed and in Done status'
    });
    ...
    const checkpoints = await manager.listWorkflowCheckpoints({ repoName: REPO_NAME, featureSlug: FEATURE_SLUG });
    ...
    const doneVerify = await manager.verifyAllTasksComplete({ repoName: REPO_NAME, featureSlug: FEATURE_SLUG });
